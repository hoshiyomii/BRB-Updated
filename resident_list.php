...
<body>
                                        ...
                                        <tr class="clickable-row" data-id="<?php echo $resident['id']; ?>">
                                            <td>
                                                <?php if (!empty($resident['profile_picture'])): ?>
                                                    <img src="<?php echo htmlspecialchars($resident['profile_picture']); ?>" alt="Profile Picture" class="rounded-circle" style="width:36px;height:36px;object-fit:cover;border:1.5px solid #ccc;">
                                                <?php else: ?>
                                                    <div class="profile-initials-list" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:#eee;border-radius:50%;font-weight:bold;color:#888;">
    ...
    <!-- Custom Resident Detail Modal (No Bootstrap) -->
    <div id="residentDetailModal" class="custom-modal-overlay" style="display:none;">
        <div class="custom-modal resident-detail-modal">
            <div class="custom-modal-header">
                <h5>Resident Details</h5>
                <button type="button" class="custom-modal-close" id="closeResidentDetailModal">&times;</button>
            </div>
            <div class="custom-modal-body">
                <div class="profile-container">
                    <img id="residentProfilePic" src="" alt="Profile Picture" class="rounded-circle" style="width: 120px; height: 120px; object-fit: cover; border: 2px solid #ccc; display: none;">
            ...
            <div class="custom-modal-footer">
                <button type="button" class="btn btn-secondary" id="closeResidentDetailModalFooter">Close</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                    ...
                    fetch(`fetch_resident.php?id=${residentId}`)
                        ...
                        .then(data => {
                            if (data.error) {
                                ...
                                document.querySelector('.custom-modal-body').innerHTML = `<div class="alert alert-danger"><strong>Error:</strong> ${data.error}</div>`;
                                document.querySelectorAll('.custom-modal-footer button:not([id^="closeResidentDetailModal"])').forEach(btn => btn.style.display = 'none');
                                return;
                            }
                            ...
                            const profilePic = document.getElementById('residentProfilePic');
                        ...
                        .catch(error => {
                            ...
                            document.querySelector('.custom-modal_body').innerHTML = `<div class="alert alert-danger"><strong>Network Error:</strong> Could not fetch resident details. Please check your connection and try again.</div>`;
                            document.querySelectorAll('.custom-modal-footer button:not([id^="closeResidentDetailModal"])').forEach(btn => btn.style.display = 'none');
                        });
                });
            });
            ...
            document.getElementById('closeResidentDetailModalFooter').addEventListener('click', closeResidentDetailModal);
            // Also close modal if clicking outside the modal content
            document.getElementById('residentDetailModal').addEventListener('click', function(e) {
                if (e.target === this) closeResidentDetailModal();
            });
